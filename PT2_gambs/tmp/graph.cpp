...
Graph :: Graph(int){
	...
	vector<int> delay_add(id_max + 1, 0);	// Valores para adicionar em node[x]->in
	do{
		arq >> id >> code >> rec_credits >> hardness >> name;

		if(id > id_max)		return;	// Arquivo zuado :/ .

		node[id] = new Nodes(code);	this->node[id]->set_name(name);
		
		getline(arq, help);
		help += '\n';
		Nodes* tmp;
		for(int i = 0, id_aux, c; (c = help[i]) != '\n'; i++){
				if(id_aux != ' ')
					id_aux = 10 * id_aux + (c - 48);
				else
					break;
			int cost = rec_credits * hardness;
			tmp = new Nodes(id_aux, cost);	tmp->set_matricula(code);	tmp->set_name(name);	// Obs: aponta p/ "copia" de noh adjacente, porem este
			this->node[id]->insert_qtd_nxt_pro(tmp, cost);												// sem grau de entrada/saida E sem os pre_req
		}
		switch(id){case 5: case 9: case 14:
					case 19: case 24: case 29:
					case 32: case 34:	getline(arq,help);}		// Quebra de linha entre os semestres
	}
	while(id != id_max);
