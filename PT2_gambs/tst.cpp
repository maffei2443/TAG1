...
*/
			...
			for(int aux; to_decrease_by_one.empty() == false;){	// Enquanto houver decrementos pendentes, decrementar
				...
				if(in_degree < 1){		// Se fez todos os pre-erq, pode entrar na ordem
					int tmp = G->get_node(aux)->get_key();
	...
	int i;
	for(i = 0; it < sorted.end(); it++, i++)	// Desfazendo o map.
		sorted[i] = *it;
	cout << "Elementos na ordem: "	  << i+1 << endl;
	return sorted;	
}
...
//	G->show();
	...
	for(int i = 1; it != sort.end(); it++, i++)
		cout << "Posicao: " << i << " Code == " << *it << " Nome : " << G->get_node(*it)->get_name() << endl;
	cout << "Eh oque??" << endl;
	return 0;
}